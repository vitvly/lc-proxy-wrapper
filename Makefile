...
CGO_CFLAGS ?= -I$(NIMBUS_ETH1_PATH)/nimcache/liblcproxy -I$(NIMBASE_H_PATH)
...
build-lc-proxy-go:
	CGO_CFLAGS="$(CGO_CFLAGS)" go build -x -v -ldflags '-v "-extldflags=-Wl,-rpath,. -llcproxy -L$(NIMBUS_ETH1_PATH)/build"'
